"""Risk management models that reshape a set of positions into target weights.

Every model takes a list of :class:`Position` and returns a mapping of symbol to
target portfolio weight, so models compose: see
...
class MaximumUnrealizedProfitPercentPerSecurity(RiskManagementModel):
    ...
    def evaluate(self, positions: List[Position]) -> Dict[str, float]:
        ...
        targets: Dict[str, float] = {}
        for p in positions:
            pnl = ((p.price - p.entry_price) / p.entry_price) * (
...
class SectorExposureRiskManagementModel(RiskManagementModel):
    ...
    def evaluate(self, positions: List[Position]) -> Dict[str, float]:
        ...
        for p in positions:
            sector_totals[p.sector] = sector_totals.get(p.sector, 0) + abs(
                weights[p.symbol]
            )
...
class MaximumOrderQuantityPercentPerSecurity(RiskManagementModel):
    ...
    def evaluate(self, positions: List[Position]) -> Dict[str, float]:
        ...
        weights = {p.symbol: (p.quantity * p.price) / nav for p in positions}
        return {
            s: max(-self.max_weight, min(self.max_weight, w))
...
class CompositeRiskManagementModel(RiskManagementModel):
    ...
    def evaluate(self, positions: List[Position]) -> Dict[str, float]:
        ...
        for model in self.models:
            ...
            nav = sum(
                current_positions[s].price * current_positions[s].quantity
