...
from systematic_trading.util.volatility import get_daily_vol
from systematic_trading.filters.filters import cusum_filter
from systematic_trading.labeling.labeling import (
    get_events,
    add_vertical_barrier,
    get_bins,
)
from systematic_trading.sampling.bootstrapping import (
    get_ind_matrix,
    get_ind_mat_label_uniqueness,
)
from systematic_trading.ensemble.sb_bagging import (
...
def _get_synthetic_samples(ind_mat, good_samples_thresh, bad_samples_thresh):
    ...
    for label in get_ind_mat_label_uniqueness(ind_mat):
        if (
            np.mean(label[label > 0]) > good_samples_thresh
            or np.mean(label[label > 0]) < bad_samples_thresh
        ):
...
class TestSequentiallyBootstrappedBagging(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data["fast_mavg"] = (
            self.data["close"].rolling(window=20, min_periods=20, center=False).mean()
        )
        self.data["slow_mavg"] = (
            self.data["close"].rolling(window=50, min_periods=50, center=False).mean()
        )
        ...
        vertical_barriers = add_vertical_barrier(
            t_events=cusum_events, close=self.data["close"], num_hours=2
        )
        ...
        self.price_bars_trim = self.data[
            ...
            & (self.data.index <= self.X_train.index.max())
        ].close
    ...
    def test_sb_classifier(self):
        ...
        self.assertTrue(
            (sb_clf.X_time_index == self.X_train.index).all()
