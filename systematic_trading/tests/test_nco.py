...
class TestNCO(unittest.TestCase):
    ...
    def test_simulate_covariance():
        ...
        np.testing.assert_almost_equal(
            mu_empir_shr.flatten(), mu_vec.flatten(), decimal=2
        )
    ...
    def test_cluster_kmeans_base(self):
        ...
        np.random.seed(0)
        corr_matrix = pd.DataFrame([[1, 0.1, -0.1], [0.1, 1, -0.3], [-0.1, -0.3, 1]])

        max_num_clusters = 2
        n_init = 10

        # Expected correlation matrix of clustered elements, clusters and, Silhouette Coefficient series
        expected_corr = pd.DataFrame([[1, 0.1, -0.1], [0.1, 1, -0.3], [-0.1, -0.3, 1]])
        ...
        corr, clusters, silh_coef = nco._cluster_kmeans_base(
            corr_matrix, max_num_clusters, n_init
        )
        ...
        np.testing.assert_almost_equal(
            np.array(corr), np.array(expected_corr), decimal=4
        )
        np.testing.assert_almost_equal(
            np.array(silh_coef), np.array(expected_silh_coef), decimal=4
        )
    ...
    def test_allocate_nco():
        ...
        w_nco_mu = nco.allocate_nco(
            cov_matrix, mu_vec=mu_vec, max_num_clusters=max_num_clusters
        )
    ...
    def test_allocate_mcos():
        ...
        w_cvo_sr_expected = pd.DataFrame(
            [
                [-0.128849, -0.326671, 0.870183, 2.020053],
                [-3.786126, -0.881858, 1.418773, 3.062546],
            ]
        )
        ...
        w_nco_sr_expected = pd.DataFrame(
            [
                [-0.204089, -0.050088, 0.912494, 1.983382],
                [-3.723231, -1.809242, 1.862001, 2.61035],
            ]
        )

        # Finding the optimal weights for minimum variance
        w_cvo, w_nco = nco.allocate_mcos(
            mu_vec, cov_mat, num_obs, num_sims, kde_bwidth, min_var_portf, lw_shrinkage
        )

        # Finding the optimal weights for maximum Sharpe ratio
        w_cvo_sr, w_nco_sr = nco.allocate_mcos(
            mu_vec,
            cov_mat,
            num_obs,
            num_sims,
        ...
        np.testing.assert_almost_equal(
            np.array(w_cvo), np.array(w_cvo_expected), decimal=6
        )
        np.testing.assert_almost_equal(
            np.array(w_nco), np.array(w_nco_expected), decimal=6
        )

        np.testing.assert_almost_equal(
            np.array(w_cvo_sr), np.array(w_cvo_sr_expected), decimal=6
        )
        np.testing.assert_almost_equal(
            np.array(w_nco_sr), np.array(w_nco_sr_expected), decimal=6
        )
    ...
    def test_estim_errors_mcos():
        ...
        err_cvo, err_nco = nco.estim_errors_mcos(
            w_cvo, w_nco, mu_vec, cov_mat, min_var_portf
        )
    ...
    def test_form_true_matrix():
        ...
        mu_expected = np.array([[0.5936214], [0.97226796], [0.8602674], [0.00681664]])

        cov_expected = pd.DataFrame(
            [
                [0.09, 0.027, 0, 0],
                [0.027, 0.09, 0, 0],
                [0, 0, 0.09, 0.027],
        ...
        mu_alt_expected = np.array(
            [[0.34260886], [0.12059827], [0.24366456], [0.17248975]]
        )

        # Finding the random vector of means and covariance matrix
        mu_vec, cov_matrix = nco.form_true_matrix(
            num_blocks, block_size, block_corr, std
        )

        # Also when the std is default
        mu_vec_alt, _ = nco.form_true_matrix(
            num_blocks, block_size, block_corr, std_alt
        )

        # Testing if the results are right
        np.testing.assert_almost_equal(mu_vec, mu_expected, decimal=4)
        np.testing.assert_almost_equal(
            np.array(cov_matrix), np.array(cov_expected), decimal=4
        )
