...
class TestLabelingOverMedian(unittest.TestCase):
    ...
    def test_resample_period(self):
        ...
        test3.drop(
            datetime.strptime("2008-01-21", "%Y-%m-%d"), inplace=True
        )  # MLK day, 2008
        ...
        test6 = excess_over_median(
            subset2, binary=True, resample_by="M", lag=False
        )  # Binary by month
        ...
        pd.testing.assert_frame_equal(
            test6, test5_actual.apply(np.sign), check_less_precise=True
        )
    ...
    def test_forward(self):
        ...
        pd.testing.assert_frame_equal(
            test8, test7_actual.apply(np.sign), check_less_precise=True
        )
        pd.testing.assert_frame_equal(test9, test9_actual, check_less_precise=True)
        pd.testing.assert_frame_equal(
            test10, test9_actual.apply(np.sign), check_less_precise=True
        )
    ...
    def test_nan(self):
        ...
        subset = self.data[cols].iloc[0:5]
        with_nan = pd.concat(
            [subset, pd.Series([np.nan] * 5, name="nan", index=subset.index)], axis=1
        )
        test11 = excess_over_median(with_nan)
        test11.drop("nan", axis=1, inplace=True)
        pd.testing.assert_frame_equal(
            test11, excess_over_median(subset), check_less_precise=True
        )
