...
class TestCh10Snippets(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dates = np.array(
            [dt.datetime(2000, 1, 1) + i * dt.timedelta(days=1) for i in range(6)]
        )
        ...
        m_discrete = np.array(
            [max(-1, min(1, m_i)) for m_i in np.round(m_signal / 0.1, 0) * 0.1]
        )
        ...
        for t_i in t_p:
            avg_list.append(
                self.events_2[(self.events_2.index <= t_i) & (self.events_2.t1 > t_i)][
                    "signal"
                ].mean()
            )
...
class TestBetSize(unittest.TestCase):
    ...
    def test_bet_size(self):
        ...
        self.assertAlmostEqual(
            m_test_sig, bet_size(w_param_sig, x_div_sig, "sigmoid"), 7
        )
...
class TestGetTPos(unittest.TestCase):
    ...
    def test_get_target_pos_sigmoid(self):
        ...
        self.assertAlmostEqual(
            pos_test, get_target_pos_sigmoid(w_param, f_i, m_p, max_pos), 7
        )
    ...
    def test_get_target_pos_power(self):
        ...
        self.assertAlmostEqual(
            pos_test, get_target_pos_power(w_param, f_i, m_p, max_pos), 7
        )
    ...
    def test_get_target_pos(self):
        ...
        max_pos_sig = 220
        pos_test_sig = int(
            max_pos_sig * (x_div_sig / np.sqrt(w_param_sig + x_div_sig * x_div_sig))
        )
        self.assertAlmostEqual(
            pos_test_sig,
            get_target_pos(w_param_sig, f_i_sig, m_p_sig, max_pos_sig, "sigmoid"),
            7,
        )
        ...
        max_pos_pow = 175
        pos_test_pow = int(
            max_pos_pow * (np.sign(x_div_pow) * abs(x_div_pow) ** w_param_pow)
        )
        self.assertAlmostEqual(
            pos_test_pow,
            get_target_pos(w_param_pow, f_i_pow, m_p_pow, max_pos_pow, "power"),
            7,
        )
...
class TestInvPrice(unittest.TestCase):
    ...
    def test_inv_price(self):
        ...
        inv_p_sig = f_i_sig - m_sig * np.sqrt(w_sig / (1 - m_sig * m_sig))
        self.assertAlmostEqual(
            inv_p_sig, inv_price(f_i_sig, w_sig, m_sig, "sigmoid"), 7
        )
...
class TestLimitPrice(unittest.TestCase):
    ...
    def test_limit_price_sigmoid(self):
        ...
        sum_inv_price_sig = sum(
            [
                ...
                for j in range(
                    abs(pos_sig + np.sign(t_pos_sig - pos_sig)), abs(t_pos_sig + 1)
                )
            ]
        )
    ...
    def test_limit_price_power(self):
        ...
        sum_inv_price_pow = sum(
            [
                ...
                for j in range(
                    abs(pos_pow + np.sign(t_pos_pow - pos_pow)), abs(t_pos_pow + 1)
                )
            ]
        )
        ...
        self.assertAlmostEqual(
            limit_p_pow,
            limit_price_power(t_pos_pow, pos_pow, f_pow, w_pow, max_pos_pow),
            7,
        )
    ...
    def test_limit_price_key_error(self):
        ...
        """
        self.assertRaises(
            KeyError, limit_price, 231, 221, 110, 3.4, 250, "NotAFunction"
        )
