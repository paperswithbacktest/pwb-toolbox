...
from systematic_trading.data_structures import standard_data_structures as ds
...
class TestDataStructures(unittest.TestCase):
    ...
    def test_dollar_bars(self):
        ...
        t_dynamic = pd.Series(
            [10000, 20000, 50000], index=[data.index[0], data.index[40], data.index[80]]
        )
        ...
        df_constant = ds.get_dollar_bars(
            self.path, threshold=t_constant, batch_size=1000, verbose=False
        )
        df_dynamic = ds.get_dollar_bars(
            self.path, threshold=t_dynamic, batch_size=1000, verbose=False
        )
        df_low = ds.get_dollar_bars(
            self.path, threshold=t_low, batch_size=1000, verbose=False
        )
    ...
    def test_volume_bars(self):
        ...
        t_dynamic = pd.Series(
            [5, 10, 30], index=[data.index[0], data.index[40], data.index[80]]
        )
        ...
        # Testing dynamic threshold size
        df_constant = ds.get_volume_bars(
            self.path, threshold=t_constant, batch_size=1000, verbose=False
        )
        df_dynamic = ds.get_volume_bars(
            self.path, threshold=t_dynamic, batch_size=1000, verbose=False
        )
        df_low = ds.get_volume_bars(
            self.path, threshold=t_low, batch_size=1000, verbose=False
        )
    ...
    def test_tick_bars(self):
        ...
        t_dynamic = pd.Series(
            [2, 5, 10], index=[data.index[0], data.index[40], data.index[80]]
        )
        ...
        # Testing dynamic threshold size
        df_constant = ds.get_tick_bars(
            self.path, threshold=t_constant, batch_size=1000, verbose=False
        )
        df_dynamic = ds.get_tick_bars(
            self.path, threshold=t_dynamic, batch_size=1000, verbose=False
        )
        df_low = ds.get_tick_bars(
            self.path, threshold=t_low, batch_size=1000, verbose=False
        )
