...
class TestLabelingFixedTime(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        test1_actual = pd.Series(
            [np.nan, -1, -1, -1, -1, 1, 1, -1, 1, -1], index=self.idx10
        )
    ...
    def test_dynamic_threshold(self):
        ...
        """
        close = self.data[["SPY", "EPP", "FXI"]][:10]
        threshold1 = pd.Series(
            [0.01, 0.005, 0, 0.01, 0.02, 0.03, 0.1, -1, 0.99, 0], index=self.idx10
        )
        ...
        test7_actual = pd.Series(
            [np.nan, 0, -1, 0, 0, 0, 0, 1, 0, -1], index=self.idx10
        )
    ...
    def test_with_standardization(self):
        ...
        """
        close = self.data[["SPY", "EPP", "FXI"]][:10]
        threshold2 = pd.Series(
            [1, 2, 0, 0.2, 0.02, 1.5, 10, -1, 500, 1], index=self.idx10
        )

        test8 = fixed_time_horizon(
            close, threshold=1, lag=False, standardized=True, window=4
        )
        test9 = fixed_time_horizon(
            close, threshold=0.1, lag=True, standardized=True, window=5
        )
        test10 = fixed_time_horizon(
            close, threshold=threshold2, lag=True, standardized=True, window=3
        )
    ...
    def test_resample(self):
        ...
        threshold3 = pd.Series(
            [0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1, 1.1], index=month_index
        )

        test11 = fixed_time_horizon(
            close1, threshold=0.02, resample_by="W", lag=True, standardized=False
        )
