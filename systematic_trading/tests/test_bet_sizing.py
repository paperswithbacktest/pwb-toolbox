...
from systematic_trading.bet_sizing.bet_sizing import (
...
from systematic_trading.bet_sizing.ch10_snippets import (
    get_signal,
    avg_active_signals,
    discrete_signal,
    get_w,
...
from systematic_trading.bet_sizing.ef3m import M2N, raw_moment, most_likely_parameters
...
class TestBetSizeProbability(unittest.TestCase):
    ...
    def test_bet_size_probability_default(self):
        ...
        events_test = pd.DataFrame(
            data=[[0.55, 1], [0.7, -1], [0.95, 1], [0.65, -1], [0.85, 1]],
            columns=["prob", "side"],
            index=dates_test,
        )
        ...
        self.assertTrue(
            signal_1.equals(
                bet_size_probability(
                    events_test, events_test["prob"], 2, events_test["side"]
                )
            )
        )
...
class TestBetSizeDynamic(unittest.TestCase):
    ...
    def test_bet_size_dynamic_default(self):
        ...
        events_results["t_pos"] = events_results.apply(
            lambda row: get_target_pos(w_param, row.f, row.m_p, row.max_pos, "sigmoid"),
            axis=1,
        )
        events_results["l_p"] = events_results.apply(
            lambda row: limit_price(
...
class TestBetSizeBudget(unittest.TestCase):
    ...
    def test_bet_size_budget_div_zero(self):
        ...
        events_result = get_concurrent_sides(events_test["t1"], events_test["side"])
        max_active_long, max_active_short = (
            events_result["active_long"].max(),
            events_result["active_short"].max(),
        )
        avg_long = (
            events_result["active_long"] / max_active_long if max_active_long > 0 else 0
        )
        avg_short = (
            events_result["active_short"] / max_active_short
...
class TestBetSizeReserve(unittest.TestCase):
    ...
    @patch("systematic_trading.bet_sizing.bet_sizing.most_likely_parameters")
    def test_bet_size_reserve_default(self, mock_likely_parameters):
        ...
        shift_dt = np.array(
            [dt.timedelta(days=d) for d in np.random.uniform(1.0, 20.0, sample_size)]
        )
        ...
        events_active["c_t"] = (
            events_active["active_long"] - events_active["active_short"]
        )
        central_moments = [
            moment(events_active["c_t"].to_numpy(), moment=i) for i in range(1, 6)
        ]
        raw_moments = raw_moment(
            central_moments=central_moments, dist_mean=events_active["c_t"].mean()
        )
        m2n_test = M2N(
            raw_moments,
            epsilon=1e-5,
        ...
        test_fit = [
            test_params[key] for key in ["mu_1", "mu_2", "sigma_1", "sigma_2", "p_1"]
        ]
...
class TestConfirmAndCastToDf(unittest.TestCase):
    ...
    def test_cast_to_df_one_series(self):
        ...
        df_results = confirm_and_cast_to_df(d_events)
        self.assertTrue(
            np.allclose(events_test.to_numpy(), df_results.to_numpy(), 1e-9)
        )
    ...
    def test_cast_to_df_no_series(self):
        ...
        events_test = pd.DataFrame(
            data=[[pos, max_pos, market_price, forecast_price]],
            columns=["pos", "max_pos", "m_p", "f"],
        )
        d_events = {
            "pos": pos,
...
class TestGetConcurrentSides(unittest.TestCase):
    ...
    def test_get_concurrent_sides_default(self):
        ...
        self.assertTrue(
            np.allclose(
                events_test.to_numpy(),
                df_results[["active_long", "active_short"]].to_numpy(),
                1e-9,
            )
        )
...
class TestCdfMixture(unittest.TestCase):
    ...
    def test_cdf_mixture_default(self):
        ...
        expected_result = (p_1 * norm.cdf(x_value, mu_1, sigma_1)) + (
            (1 - p_1) * norm.cdf(x_value, mu_2, sigma_2)
        )
        # Evaluate.
...
class TestSingleBetSizeMixed(unittest.TestCase):
    ...
    def test_single_bet_size_mixed_above_zero(self):
        ...
        expected_result = (cdf_mixture(c_t, params) - cdf_mixture(0, params)) / (
            1 - cdf_mixture(0, params)
        )
        # Evaluate.
    ...
    def test_single_bet_size_mixed_below_zero(self):
        ...
        # Calculate the expected result.
        expected_result = (
            cdf_mixture(c_t, params) - cdf_mixture(0, params)
        ) / cdf_mixture(0, params)
        # Evaluate.
        self.assertEqual(expected_result, single_bet_size_mixed(c_t, params))
