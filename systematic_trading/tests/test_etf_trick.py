...
from systematic_trading.multi_product.etf_trick import ETFTrick
...
class TestETFTrick(unittest.TestCase):
    ...
    def test_etf_trick_costs_defined(self):
        ...
        in_memory_etf_trick = ETFTrick(
            self.open_df, self.close_df, self.alloc_df, self.costs_df, self.rates_df
        )
        ...
        self.assertTrue(
            in_memory_trick_series.iloc[-1] == csv_trick_series_100.iloc[-1]
        )
        self.assertTrue(
            in_memory_trick_series.iloc[-1] == csv_trick_series_all.iloc[-1]
        )

        # Assert same values
        self.assertTrue(
            np.all(in_memory_trick_series.values == csv_trick_series_4.values)
        )
        self.assertTrue(
            np.all(in_memory_trick_series.values == csv_trick_series_100.values)
        )
        self.assertTrue(
            np.all(in_memory_trick_series.values == csv_trick_series_all.values)
        )
    ...
    def test_etf_trick_rates_not_defined(self):
        ...
        in_memory_etf_trick = ETFTrick(
            self.open_df, self.close_df, self.alloc_df, self.costs_df, None
        )
        in_memory_trick_series = in_memory_etf_trick.get_etf_series()
        csv_trick_series_4 = csv_etf_trick.get_etf_series(batch_size=4)
        csv_etf_trick.reset()
    ...
    def test_input_exceptions(self):
        ...
        try:
            ETFTrick(
                modified_open_df, self.close_df, self.alloc_df, self.costs_df, None
            )
        except ValueError:
            pass
