...
from systematic_trading.backtest_statistics.statistics import (
...
class TestBacktestStatistics(unittest.TestCase):
    ...
    def test_all_bets_concentration(self):
        ...
        self.assertAlmostEqual(all_returns_concentration[0], 0.0014938, delta=1e-5)
        self.assertAlmostEqual(all_returns_concentration[1], 0.0016261, delta=1e-5)
        self.assertAlmostEqual(all_returns_concentration[2], 0.0195998, delta=1e-5)
    ...
    def test_drawdown_and_time_under_water(self):
        ...
        drawdown_dol, time_under_water_dol = drawdown_and_time_under_water(
            self.dollar_returns, dollars=True
        )
        _, time_under_water = drawdown_and_time_under_water(
            self.dollar_returns / 100, dollars=False
        )

        self.assertTrue(list(drawdown_dol) == [20.0, 30.0, 10.0])
        self.assertTrue(list(time_under_water) == list(time_under_water_dol))
        self.assertAlmostEqual(time_under_water[0], 0.010951, delta=1e-4)
        self.assertAlmostEqual(time_under_water[1], 0.008213, delta=1e-4)
    ...
    def test_probabilistic_sharpe_ratio(self):
        ...
        result_prob_sr = probabilistic_sharpe_ratio(
            observed_sr, benchmark_sr, number_of_returns, skewness, kurtosis
        )

        self.assertAlmostEqual(result_prob_sr, 0.95727, delta=1e-4)
    ...
    def test_deflated_sharpe_ratio(self):
        ...
        result_defl_sr = deflated_sharpe_ratio(
            observed_sr, sr_estimates, number_of_returns, skewness, kurtosis
        )

        benchmark_sr = deflated_sharpe_ratio(
            observed_sr,
            estim_param,
            number_of_returns,
            skewness,
        ...
        self.assertAlmostEqual(benchmark_sr, 1.012241, delta=1e-4)

        self.assertAlmostEqual(param_defl_sr, 0.941740, delta=1e-4)
    ...
    def test_minimum_track_record_length(self):
        ...
        result_min_track_rec = minimum_track_record_length(
            observed_sr, benchmark_sr, skewness, kurtosis, alpha
        )

        self.assertAlmostEqual(result_min_track_rec, 228.73497, delta=1e-4)
