...
from systematic_trading.feature_importance import (
...
class TestModelFingerprint(unittest.TestCase):
    ...
    def test_linear_effect(self):
        ...
        for feature, effect_value in zip(
            informative_features_1, [0.0688, 0.5292, 0.125, 0.2117]
        ):
            self.assertAlmostEqual(
                linear_effect["norm"][feature], effect_value, delta=1e-3
            )
        ...
        for feature, effect_value in zip(
            informative_features_2, [0.13, 0.0477, 0.1, 0.4, 0.208]
        ):
            self.assertAlmostEqual(
                linear_effect["norm"][feature], effect_value, delta=1e-3
            )
        ...
        for feature in informative_features_1:
            self.assertAlmostEqual(
                linear_effect["norm"][feature],
                linear_effect_70["norm"][feature],
                delta=0.05,
            )
    ...
    def test_non_linear_effect(self):
        ...
        # due to scikit-learn changing RandomForestRegressor, and RandomForestClassifier outputs
        for feature, effect_value in zip(
            informative_features_1, [0.0858, 0.3972, 0.0857, 0.2663]
        ):
            self.assertAlmostEqual(
                non_linear_effect["norm"][feature], effect_value, delta=1e-3
            )
        ...
        for feature in informative_features_1:
            self.assertAlmostEqual(
                non_linear_effect["raw"][feature],
                non_linear_effect_70["raw"][feature],
                delta=0.05,
            )
    ...
    def test_pairwise_effect(self):
        ...
        self.reg_fingerprint.fit(
            self.reg_rf, self.X, num_values=20, pairwise_combinations=combinations
        )
        ...
        # due to scikit-learn changing RandomForestRegressor, and RandomForestClassifier outputs
        for pair, effect_value in zip(
            combinations, [0.188, 0.15792, 0.005, 0.0299, 0, 0.00162]
        ):
            self.assertAlmostEqual(
                pair_wise_effect["raw"][str(pair)], effect_value, delta=1e-3
            )

        combinations = [(0, 5), (0, 12), (1, 2), (5, 7), (3, 6), (4, 9)]
        self.reg_fingerprint.fit(
            self.reg_linear, self.X, num_values=20, pairwise_combinations=combinations
        )
        _, _, pair_wise_effect = self.reg_fingerprint.get_effects()
    ...
    def test_classification_fingerpint(self):
        ...
        clf_fingerpint.fit(
            clf, X, num_values=20, pairwise_combinations=[(0, 1), (2, 3), (8, 9)]
        )
        ...
        for feature, effect in zip([0, 2, 3, 8, 9], [0.0062, 0.0217, 0.0155, 0.0013]):
            self.assertAlmostEqual(
                non_linear_effect["raw"][feature], effect, delta=1e-3
            )

        for comb, effect in zip([(0, 1), (2, 3), (8, 9)], [0.008, 0.0087, 0]):
            self.assertAlmostEqual(
                pair_wise_effect["raw"][str(comb)], effect, delta=1e-3
            )
    ...
    def test_plot_effects(self):
        ...
        self.reg_fingerprint.fit(
            self.reg_rf, self.X, num_values=20, pairwise_combinations=[(1, 2), (3, 5)]
        )
        self.reg_fingerprint.plot_effects()
