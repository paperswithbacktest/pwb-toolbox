...
import unittest
import pandas as pd
from systematic_trading.cross_validation import CombinatorialPurgedKFold
...
class TestCombinatorialPurgedCV(unittest.TestCase):
    ...
    def test_test_times(self):
        ...
        cv_gen = CombinatorialPurgedKFold(
            n_splits=6, n_test_splits=2, samples_info_sets=self.info_sets
        )
        ...
        self.assertEqual(
            counter, 15
        )  # Example from the book says that for CombinatorialCV(6, 2) number of splits=15

        # Check indices from example from the book (page 164)
        for test_idx, [start_idx, end_idx] in zip(
            test_splits[0:5], [[0, 33], [0, 50], [0, 67], [0, 83], [0, 99]]
        ):
            self.assertEqual(test_idx[0], start_idx)
            self.assertEqual(test_idx[-1], end_idx)

        for test_idx, [start_idx, end_idx] in zip(
            test_splits[-5:], [[34, 83], [34, 99], [51, 83], [51, 99], [68, 99]]
        ):
            self.assertEqual(test_idx[0], start_idx)
            self.assertEqual(test_idx[-1], end_idx)
        ...
        # Test backtest paths
        self.assertEqual(
            len(cv_gen.backtest_paths), cv_gen.n_splits - 1
        )  # phi[N, 2] = N - 1
        ...
        self.assertTrue(
            (cv_gen.backtest_paths[-1][-1]["train"] == train_splits[-1]).all()
        )
        self.assertTrue(
            (cv_gen.backtest_paths[-1][-2]["train"] == train_splits[-1]).all()
        )
    ...
    def test_purge_and_embargo(self):
        ...
        cv_gen = CombinatorialPurgedKFold(
            n_splits=6,
            n_test_splits=2,
            samples_info_sets=self.info_sets,
            pct_embargo=0.05,
        )
        train_splits = []
        test_splits = []
        ...
        cv_gen_no_embargo = CombinatorialPurgedKFold(
            n_splits=6,
            n_test_splits=2,
            samples_info_sets=self.info_sets,
            pct_embargo=0.0,
        ...
        self.assertEqual(
            len(train_splits_no[0]) - len(train_splits[0]), 5
        )  # Embargo of 1 train set
        self.assertEqual(
            len(train_splits_no[1]) - len(train_splits[1]), 10
        )  # Embargo of 2 train set
        self.assertEqual(
            len(train_splits_no[-1]) - len(train_splits[-1]), 0
        )  # Latest set is not embargo-ed
    ...
    def test_errors_raise(self):
        ...
        with self.assertRaises(ValueError):
            CombinatorialPurgedKFold(
                n_splits=6, n_test_splits=3, samples_info_sets=[1, 2, 3, 4]
            )
