...
from systematic_trading.data_structures import run_data_structures as ds
...
class TestDataStructures(unittest.TestCase):
    ...
    def test_csv_format(self):
        ...
        self.assertRaises(
            ValueError, ds.BaseRunBars._assert_csv, pd.DataFrame(wrong_date).T
        )
        # pylint: disable=protected-access
        self.assertRaises(
            AssertionError, ds.BaseRunBars._assert_csv, pd.DataFrame(too_many_cols).T
        )
        # pylint: disable=protected-access
        self.assertRaises(
            AssertionError, ds.BaseRunBars._assert_csv, pd.DataFrame(wrong_price).T
        )
        # pylint: disable=protected-access
        self.assertRaises(
            AssertionError, ds.BaseRunBars._assert_csv, pd.DataFrame(wrong_volume).T
        )
