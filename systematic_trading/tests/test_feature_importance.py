...
from systematic_trading.clustering.feature_clusters import get_feature_clusters
...
class TestFeatureImportance(unittest.TestCase):
    ...
    def setUp(self):
        ...
        """
        # Generate data sets
        self.X, self.y = get_classification_data(
            10, 5, 2, 1000, random_state=0, sigma=0
        )
        # Fit a RF
        ...
        self.bag_clf = BaggingClassifier(
            base_estimator=self.clf_base,
            max_features=1.0,
            n_estimators=100,
            oob_score=True,
            random_state=1,
    ...
    def test_orthogonal_features(self):
        ...
        self.assertAlmostEqual(
            pca_corr_res["Weighted_Kendall_Rank"][0], 0.7424, delta=1e-1
        )
    ...
    def test_feature_importance(self):
        ...
        clustered_mdi = mean_decrease_impurity(
            self.fit_clf, self.X.columns, clustered_subsets=clustered_subsets_linear
        )
        mdi_cfi_single = mean_decrease_impurity(
            self.fit_clf, self.X.columns, clustered_subsets=feature_subset_single
        )
        ...
        mda_feat_imp_f1 = mean_decrease_accuracy(
            self.bag_clf, self.X, self.y, self.cv_gen, scoring=f1_score
        )
        ...
        self.assertAlmostEqual(
            mda_feat_imp_log_loss.loc["I_1", "mean"], 0.65522, delta=0.1
        )
        # Value in the below test was changed after v.0.11.3 from 0.00332
        #                                                     to -0.20645
        # due to scikit-learn changing RandomForestClassifier, and BaggingClassifier outputs
        self.assertAlmostEqual(
            mda_feat_imp_log_loss.loc["R_0", "mean"], -0.20645, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            sfi_feat_imp_log_loss.loc["I_0", "mean"], -6.39442, delta=0.1
        )
        self.assertAlmostEqual(
            sfi_feat_imp_log_loss.loc["R_0", "mean"], -5.04315, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            mdi_feat_imp.loc["I_1", "mean"],
            mdi_cfi_single.loc["I_1", "mean"],
            delta=0.1,
        )
        self.assertAlmostEqual(
            mdi_feat_imp.loc["R_0", "mean"],
            mdi_cfi_single.loc["R_0", "mean"],
            delta=0.1,
        )
        self.assertAlmostEqual(
            mda_feat_imp_log_loss.loc["I_1", "mean"],
            mda_cfi_single.loc["I_1", "mean"],
            delta=0.1,
        )
        self.assertAlmostEqual(
            mda_feat_imp_log_loss.loc["R_0", "mean"],
    ...
    def test_plot_feature_importance(self):
        ...
        oos_score = cross_val_score(
            self.bag_clf, self.X, self.y, cv=self.cv_gen, scoring="accuracy"
        ).mean()

        mdi_feat_imp = mean_decrease_impurity(self.bag_clf, self.X.columns)
        plot_feature_importance(
            mdi_feat_imp, oob_score=self.bag_clf.oob_score_, oos_score=oos_score
        )
        plot_feature_importance(
            mdi_feat_imp,
            oob_score=self.bag_clf.oob_score_,
            oos_score=oos_score,
            save_fig=True,
