...
from systematic_trading.data_structures import get_volume_bars
from systematic_trading.microstructural_features import (
    get_vpin,
...
from systematic_trading.microstructural_features.encoding import encode_tick_rule_array
...
from systematic_trading.util import get_bvc_buy_volume
...
class TestMicrostructuralFeatures(unittest.TestCase):
    ...
    def test_first_generation(self):
        ...
        corwin_schultz = get_corwin_schultz_estimator(
            self.data.high, self.data.low, window=20
        )
        bekker_parkinson = get_bekker_parkinson_vol(
            self.data.high, self.data.low, window=20
        )
        ...
        self.assertAlmostEqual(
            roll_measure[25], 1.176, delta=1e-3
    ...
    def test_second_generation_intra_bar(self):
        ...
        """
        kyle_lambda = get_bar_based_kyle_lambda(
            self.data.close, self.data.cum_vol, window=20
        )
        amihud_lambda = get_bar_based_amihud_lambda(
            self.data.close, self.data.cum_dollar, window=20
        )
        hasbrouck_lambda = get_bar_based_hasbrouck_lambda(
            self.data.close, self.data.cum_dollar, window=20
        )
        ...
        self.assertAlmostEqual(
            kyle_lambda[25], 7.76e-5, delta=1e-6
        )  # Test some random value
    ...
    def test_third_generation(self):
        ...
        """
        bvc_buy_volume = get_bvc_buy_volume(
            self.data.close, self.data.cum_vol, window=20
        )
    ...
    def test_feature_generator_function(self):
        ...
        df_trades["log_ret"] = np.log(
            df_trades.Price / df_trades.Price.shift(1)
        ).dropna()
        non_null_log_ret = df_trades[df_trades.log_ret != 0].log_ret.dropna()

        # Take unique volumes only
        volume_mapping = quantile_mapping(
            df_trades.Volume.drop_duplicates(), num_letters=10
        )
        ...
        self.assertTrue(
            (features.dropna().values == features_from_df.dropna().values).all()
        )
        ...
        self.assertAlmostEqual(
            (features - features_from_csv).sum().sum(), 0, delta=1e-6
        )

        self.assertEqual(bar_index.shape[0], features.shape[0])
        self.assertEqual(
            compressed_bars.loc[features.index].shape[0], compressed_bars.shape[0]
        )
    ...
    def test_inter_bar_feature_values(self):
        ...
        self.assertAlmostEqual(
            features.tick_rule_entropy_shannon.max(), 1.52192, delta=1e-4
        )
        self.assertAlmostEqual(
            features.tick_rule_entropy_shannon.mean(), 0.499, delta=1e-4
        )
        self.assertAlmostEqual(
            features.tick_rule_entropy_shannon[3], 0.39124, delta=1e-4
        )

        # Volume entropy plug-in
        self.assertAlmostEqual(
            features.volume_entropy_plug_in.max(), 1.92192, delta=1e-4
        )
        self.assertAlmostEqual(
            features.volume_entropy_plug_in.mean(), 1.052201, delta=1e-5
        )
        self.assertAlmostEqual(features.volume_entropy_plug_in[3], 0.41381, delta=1e-4)

        # Volume entropy Lempel-Ziv
        self.assertAlmostEqual(
            features.volume_entropy_lempel_ziv.max(), 1.0, delta=1e-4
        )
        self.assertAlmostEqual(
            features.volume_entropy_lempel_ziv.mean(), 0.5904612, delta=1e-4
        )
        self.assertAlmostEqual(
            features.volume_entropy_lempel_ziv[3], 0.46153, delta=1e-4
        )

        # Pct entropy Lempel-Ziv
        self.assertAlmostEqual(features.pct_entropy_lempel_ziv.max(), 0.8, delta=1e-4)
        self.assertAlmostEqual(
            features.pct_entropy_lempel_ziv.mean(), 0.56194, delta=1e-5
        )
        ...
        self.assertAlmostEqual(
            features.pct_entropy_konto.mean(), 0.83039791, delta=1e-5
        )
