...
class TestTIC(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.price_data = pd.read_csv(
            price_data_path, parse_dates=True, index_col="Date"
        )
    ...
    def test_get_linkage_corr(self):
        ...
        np.testing.assert_almost_equal(
            np.array(pd.DataFrame(dendrogram)),
    ...
    def test_link_clusters():
        ...
        lnk0 = np.array([])
        lnk1 = np.array(
            [[1, 3, 0.10526126, 2], [4, 2, 0.23105119, 3], [0, 5, 0.40104189, 4]]
        )
        items0 = [1010, 1020, 2030, 1040, 1030]
        items1 = [1010, 1020, 1040, 1030]

        # Expected link array
        link_expected = np.array(
            [[1, 4, 0.10526126, 2], [5, 3, 0.23105119, 3], [0, 6, 0.40104189, 4]]
        )
    ...
    def test_update_dist():
        ...
        # Creating a sample input
        dist0 = pd.DataFrame(
            [[0.0, 0.595671], [0.595671, 0.0]], columns=[20, 10], index=[20, 10]
        )
        lnk0 = np.array(
            [
                [1, 4, 0.10526126, 2],
        ...
        dist_new_alt = tic._update_dist(
            dist0, lnk0, lnk_, items0, criterion=alt_criterion
        )

        # Testing that the obtained distance matrix is right
        np.testing.assert_almost_equal(
            np.array(dist_new), np.array(dist_expected), decimal=2
        )

        # Testing that the obtained distance matrix is right
        np.testing.assert_almost_equal(
            np.array(dist_new_alt), np.array(dist_expected), decimal=2
        )
    ...
    def test_link2corr(self):
        ...
        np.testing.assert_almost_equal(
            np.array(corr), np.array(corr_expected), decimal=2
        )
    ...
    def test_tic_correlation(self):
        ...
        # Getting the correlation matrix
        corr = tic.tic_correlation(
            etf_classification_tree, etf_corr, tn_relation=tn_relation, kde_bwidth=0.25
        )
