...
class TestRiskEstimators(unittest.TestCase):
    ...
    def test_fit_kde(self):
        ...
        pdf_kde = risk_estimators._fit_kde(
            observations, eval_points=eval_points, kde_bwidth=0.25
        )
    ...
    def test_pdf_fit(self):
        ...
        # Calculating the SSE
        pdf_kde = risk_estimators._pdf_fit(
            var, eigen_observations, tn_relation, kde_bwidth
        )
    ...
    def test_corr_to_cov():
        ...
        corr_matrix = np.array([[1, 0.1, -0.1], [0.1, 1, -0.3], [-0.1, -0.3, 1]])
        std_vec = np.array([0.1, 0.2, 0.1])

        # Expected covariance matrix
        expected_matrix = np.array(
            [[0.01, 0.002, -0.001], [0.002, 0.04, -0.006], [-0.001, -0.006, 0.01]]
        )
    ...
    def test_cov_to_corr():
        ...
        expected_matrix = np.array([[1, 0.1, -0.1], [0.1, 1, -0.3], [-0.1, -0.3, 1]])
    ...
    def test_get_pca():
        ...
        # Expected correlation matrix
        expected_eigenvalues = np.array([[1.3562, 0, 0], [0, 0.9438, 0], [0, 0, 0.7]])
    ...
    def test_denoise_covariance():
        ...
        expected_cov_alt = np.array(
            [[0.01, 0.0057, -0.0028], [0.0057, 0.04, -0.0106], [-0.0028, -0.0106, 0.01]]
        )

        expected_cov_detoned = np.array(
            [
                [0.01, -0.00672445, 0.00336222],
        ...
        np.testing.assert_almost_equal(
            cov_matrix_denoised_alt, expected_cov_alt, decimal=4
        )

        # Testing if the de-toned covariance matrix is right
        np.testing.assert_almost_equal(
            cov_matrix_detoned, expected_cov_detoned, decimal=4
        )
    ...
    def test_minimum_covariance_determinant(self):
        ...
        expected_cov = np.array(
            [
                [1.5110e-04, 1.1322e-04, -5.2053e-06],
        ...
        np.testing.assert_almost_equal(
            min_covar_determ, min_covar_determ_ret, decimal=4
        )
    ...
    def test_empirical_covariance(self):
        ...
        expected_cov = np.array(
            [
                [4.6571e-04, 3.4963e-04, -1.6626e-05],
        ...
        empirical_cov = risk_estimators.empirical_covariance(
            prices_dataframe, price_data=True
        )
    ...
    def test_shrinked_covariance(self):
        ...
        shrinked_cov_all = risk_estimators.shrinked_covariance(
            prices_dataframe, price_data=True, shrinkage_type="all", basic_shrinkage=0.1
        )
        ...
        np.testing.assert_almost_equal(
            shrinked_cov_basic, expected_cov_basic, decimal=7
        )
        ...
        np.testing.assert_almost_equal(
            shrinked_cov_all[0], shrinked_cov_basic, decimal=7
        )
        ...
        # And if the results for price and returns are the same
        np.testing.assert_almost_equal(
            shrinked_cov_basic, shrinked_cov_basic_ret, decimal=4
        )
    ...
    def test_semi_covariance(self):
        ...
        semi_cov = risk_estimators.semi_covariance(
            prices_dataframe, price_data=True, threshold_return=0
        )

        # Calculating the Semi-Covariance matrix on returns data with zero threshold (volatility of negative returns)
        semi_cov_ret = risk_estimators.semi_covariance(
            returns_dataframe, price_data=False, threshold_return=0
        )
        ...
        # And if the results for price and returns are the same
        np.testing.assert_almost_equal(
            np.array(semi_cov), np.array(semi_cov_ret), decimal=4
        )
    ...
    def test_exponential_covariance(self):
        ...
        expon_cov = risk_estimators.exponential_covariance(
            prices_dataframe, price_data=True, window_span=60
        )
        ...
        # And if the results for price and returns are the same
        np.testing.assert_almost_equal(
            np.array(expon_cov), np.array(expon_cov_ret), decimal=4
        )
