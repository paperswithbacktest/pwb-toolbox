...
class TestReturnOverBenchmark(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        pd.testing.assert_series_equal(
            test2, test1_actual.apply(np.sign), check_names=False
        )
    ...
    def test_given_benchmark(self):
        ...
        benchmark4 = pd.Series(
            [0, 0.01, -0.01, 0.02, -0.005, 0.6, 100, -90, -0.2, 0.008], index=self.idx10
        )
        data4 = self.data["BND"]
        test4 = return_over_benchmark(data4[:10], benchmark=benchmark4, lag=False)
        test5 = return_over_benchmark(
            data4[:10], benchmark=benchmark4, binary=True, lag=False
        )
        test4_actual = pd.Series(
            [
                np.nan,
                -8.70736203e-03,
                1.11619412e-02,
    ...
    def test_resample(self):
        ...
        subset1 = data5[40:50]
        subset2 = data5[0:130]
        benchmark_day = pd.Series(
            [0.01, 0.01, 0.01, -0.01, -0.01, -0.02, 0.2, 0.04, -0.1, 0],
            index=subset1.index,
        )
        month_index = subset2.resample("M").last().index

        test7 = return_over_benchmark(
            subset1, benchmark=benchmark_day, binary=False, resample_by="B", lag=True
        )
        test7b = return_over_benchmark(
            subset1, benchmark=benchmark_day, binary=False, lag=True
        )
        test8 = return_over_benchmark(
            subset2, benchmark=-0.02, binary=True, resample_by="M", lag=True
        )  # Negative
        test8_actual = pd.DataFrame(
            {
                "EEM": [1, -1, 1, 1, -1, -1, np.nan],
                "EWG": [1, 1, 1, 1, -1, 1, np.nan],
                "TIP": [1, 1, -1, 1, 1, 1, np.nan],
            },
            index=month_index,
        )
