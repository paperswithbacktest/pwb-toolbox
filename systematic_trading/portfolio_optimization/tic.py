...
class TIC:
    ...
    def tic_correlation(self, tree_struct, corr_matrix, tn_relation, kde_bwidth=0.01):
        ...
        ti_correlation_denoised = risk_estim.denoise_covariance(
            ti_correlation, tn_relation=tn_relation, kde_bwidth=kde_bwidth
        )
    ...
    def _get_linkage_corr(self, tree_struct, corr_matrix):
        ...
        tree_levels = [
            [tree_struct.columns[i - 1], tree_struct.columns[i]]
            for i in range(1, tree_struct.shape[1])
        ]
        ...
        for level in tree_levels:
            ...
            for high_element, grouped_elements in grouped_level:
                ...
                if len(grouped_elements) == 1:
                    ...
                    global_elements[
                        global_elements.index(grouped_elements[0])
                    ] = high_element

                    # Changing the name also in the correlation matrix rows and columns
                    distance_matrix = distance_matrix.rename(
                        {grouped_elements[0]: high_element}, axis=0
                    )
                    distance_matrix = distance_matrix.rename(
                        {grouped_elements[0]: high_element}, axis=1
                    )
                ...
                local_linkage_transformed = self._link_clusters(
                    global_linkage, local_linkage, global_elements, grouped_elements
                )

                # Adding new link elements to the general linage object
                global_linkage = np.append(
                    global_linkage, local_linkage_transformed, axis=0
                )

                # As more clusters were created, their names are added to the global list of elements
                global_elements += range(
                    len(global_elements),
                    len(global_elements) + len(local_linkage_transformed),
                )
                ...
                distance_matrix = self._update_dist(
                    distance_matrix,
                    global_linkage,
                    local_linkage_transformed,
                    global_elements,
                )
                ...
                distance_matrix.columns = distance_matrix.columns[:-1].tolist() + [
                    high_element
                ]
        ...
        global_linkage = np.array(
            [*map(tuple, global_linkage)],
    ...
    def _link_clusters(
        ...
        for link in range(local_linkage_tr.shape[0]):
            ...
            for j in range(2):
                ...
                if local_linkage_tr[link, j] < len(
                    grouped_elements
                ):  # If it's the element from the grouped ones
                    # Then replacing its local name with the actual name from the list of all elements' names
                    local_linkage_tr[link, j] = global_elements.index(
                        grouped_elements[int(local_linkage_tr[link, j])]
                    )
                    ...
                    local_linkage_tr[link, j] += -len(grouped_elements) + len(
                        global_elements
                    )
                ...
                if (
                    local_linkage_tr[link, j] < num_atoms
                    ...
                    if local_linkage_tr[link, j] - num_atoms < global_linkage.shape[0]:
                        ...
                        atom_counter += global_linkage[
                            int(local_linkage_tr[link, j]) - num_atoms, 3
                        ]
                        ...
                        atom_counter += local_linkage_tr[
                            int(local_linkage_tr[link, j]) - len(global_elements), 3
                        ]
    ...
    def _update_dist(
        ...
        for i in range(local_linkage_tr.shape[0]):
            ...
            elem_1, elem_2 = (
                global_elements[int(local_linkage_tr[i, 0])],
                global_elements[int(local_linkage_tr[i, 1])],
            )
            ...
            if criterion is None:
                ...
                if (
                    local_linkage_tr[i, 0] < num_atoms
                    ...
                    elem_1_weight = global_linkage[
                        int(local_linkage_tr[i, 0]) - num_atoms, 3
                    ]

                if (
                    local_linkage_tr[i, 1] < num_atoms
                    ...
                    # Weight is set to the number of atoms in a cluster
                    elem_2_weight = global_linkage[
                        int(local_linkage_tr[i, 1]) - num_atoms, 3
                    ]

                # Calculating new distance as the average weighted distance
                # where the weight is the number of atoms in an element
                dist_vector = (
                    distance_matrix[elem_1] * elem_1_weight
                    + distance_matrix[elem_2] * elem_2_weight
    ...
    def _link2corr(self, linkage, element_index):
        ...
        corr_matrix = pd.DataFrame(
            np.eye(linkage.shape[0] + 1),
            index=element_index,
            columns=element_index,
            dtype=float,
        )
        ...
        for link in range(linkage.shape[0]):
            ...
            corr_matrix.loc[element_index[el_x], element_index[el_y]] = (
                1 - 2 * linkage["dist"][link] ** 2
            )

            # And the symmetrical values
            corr_matrix.loc[element_index[el_y], element_index[el_x]] = (
                1 - 2 * linkage["dist"][link] ** 2
            )
