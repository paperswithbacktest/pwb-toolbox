...
class NCO:
    ...
    def allocate_nco(self, cov, mu_vec=None, max_num_clusters=None, n_init=10):
        ...
        corr, clusters, _ = self._cluster_kmeans_base(
            corr, max_num_clusters, n_init=n_init
        )
        ...
        w_inter_clusters = pd.Series(
            self.allocate_cvo(cov_inter_cluster, mu_inter_cluster).flatten(),
            index=cov_inter_cluster.index,
        )

        # Final allocations - dot-product of the intra-cluster and inter-cluster allocations (step 4)
        w_nco = (
            w_intra_clusters.mul(w_inter_clusters, axis=1)
            .sum(axis=1)
            .values.reshape(-1, 1)
        )
    ...
    def allocate_mcos(
        ...
        w_cvo = pd.DataFrame(
            columns=range(cov.shape[0]), index=range(num_sims), dtype=float
        )
        ...
        for simulation in range(num_sims):
            ...
            if kde_bwidth > 0:
                cov_simulation = risk_estimators.denoise_covariance(
                    cov_simulation, num_obs / cov_simulation.shape[1], kde_bwidth
                )

            # Writing the results to corresponding dataframes
            w_cvo.loc[simulation] = self.allocate_cvo(
                cov_simulation, mu_simulation
            ).flatten()
            w_nco.loc[simulation] = self.allocate_nco(
                cov_simulation, mu_simulation, int(cov_simulation.shape[0] / 2)
            ).flatten()
    ...
    def _simulate_covariance(mu_vector, cov_matrix, num_obs, lw_shrinkage=False):
        ...
        observations = np.random.multivariate_normal(
            mu_vector.flatten(), cov_matrix, size=num_obs
        )
    ...
    def _cluster_kmeans_base(corr, max_num_clusters=None, n_init=10):
        ...
        clusters = {
            i: corr.columns[np.where(kmeans_optimal.labels_ == i)[0]].tolist()
            for i in np.unique(kmeans_optimal.labels_)
        }
    ...
    def form_true_matrix(self, num_blocks, block_size, block_corr, std=None):
        ...
        if std is None:  # Default intra-cluster correlations at 0.5
            std = np.random.uniform(0.05, 0.2, corr_matrix.shape[0])
