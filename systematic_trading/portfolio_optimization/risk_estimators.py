...
class RiskEstimators:
    ...
    def shrinked_covariance(
        ...
        if shrinkage_type == "basic":
            cov_matrix = (
                ShrunkCovariance(
                    assume_centered=assume_centered, shrinkage=basic_shrinkage
                )
                .fit(returns)
                .covariance_
            )
    ...
    def denoise_covariance(
        ...
        maximum_eigen, _ = self._find_max_eval(
        ...
        num_facts = eigenval.shape[0] - np.diag(eigenval)[::-1].searchsorted(
            maximum_eigen
        )
        ...
        if detone:
            corr = self._detoned_corr(
                corr, eigenval, eigenvec, num_facts, market_component
            )
    ...
    @staticmethod
    def _fit_kde(
        observations, kde_bwidth=0.01, kde_kernel="gaussian", eval_points=None
    ):
        """
    ...
    def _mp_pdf(var, tn_relation, num_points):
        ...
        pdf = (
            tn_relation
            * ((eigen_max - eigen_space) * (eigen_space - eigen_min)) ** (1 / 2)
    ...
    def _pdf_fit(
        self, var, eigen_observations, tn_relation, kde_bwidth, num_points=1000
    ):
        """
    ...
    def _detoned_corr(
        self, corr, eigenvalues, eigenvectors, num_facts, market_component=1
    ):
        """
