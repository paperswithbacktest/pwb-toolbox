...
class MeanVarianceOptimisation:
    ...
    def allocate(
        ...
        if solution == "inverse_variance":
            self._inverse_variance(
                covariance=covariance, expected_returns=expected_asset_returns
            )
            ...
            self._max_diversification(
                covariance=covariance, expected_returns=expected_asset_returns
            )
        else:
            self._max_decorrelation(
                covariance=covariance, expected_returns=expected_asset_returns
            )

        # Calculate the portfolio sharpe ratio
        self.portfolio_sharpe_ratio = (self.portfolio_return - self.risk_free_rate) / (
    ...
    def allocate_custom_objective(
        self, non_cvxpy_variables, cvxpy_variables, objective_function, constraints=None
    ):
        ...
        problem = cp.Problem(
            objective=allocation_objective, constraints=allocation_constraints
        )
    ...
    def plot_efficient_frontier(
        ...
        plt.scatter(
            volatilities[max_sharpe_ratio_index],
        ...
        plt.scatter(
            volatilities[min_volatility_index],
    ...
    def _calculate_estimators(
        self, asset_prices, expected_asset_returns, covariance_matrix
    ):
        """
        ...
        if expected_asset_returns is None:
            if self.calculate_expected_returns == "mean":
                expected_asset_returns = (
                    self.returns_estimator.calculate_mean_historical_returns(
                        asset_prices=asset_prices
                    )
                )
            elif self.calculate_expected_returns == "exponential":
                expected_asset_returns = (
                    self.returns_estimator.calculate_exponential_historical_returns(
                        asset_prices=asset_prices
                    )
                )
            else:
        ...
        cov = pd.DataFrame(
            covariance_matrix, index=self.asset_names, columns=self.asset_names
        )
    ...
    def _inverse_variance(self, covariance, expected_returns):
        ...
        ivp = 1.0 / np.diag(covariance)
        ivp /= ivp.sum()
        self.weights = ivp
        self.portfolio_risk = np.dot(
            self.weights, np.dot(covariance.values, self.weights.T)
        )
    ...
    def _max_sharpe(self, covariance, expected_returns):
        ...
        if isinstance(self.weight_bounds, tuple):
            allocation_constraints.extend(
                [y >= kappa * self.weight_bounds[0], y <= kappa * self.weight_bounds[1]]
            )
        else:
            for inequality in self.weight_bounds:
                allocation_constraints.append(eval(inequality))
    ...
    def _min_volatility_for_target_return(
        self, covariance, expected_returns, target_return
    ):
        # pylint: disable=eval-used
        """
