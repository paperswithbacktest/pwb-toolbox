...
class HierarchicalEqualRiskContribution:
    ...
    def allocate(
        ...
        if asset_returns is None and (
            risk_measure in {"expected_shortfall", "conditional_drawdown_risk"}
            or covariance_matrix is None
            or not optimal_num_clusters
        ):
        ...
        if not optimal_num_clusters:
            ...
            self.optimal_num_clusters = self._check_max_number_of_clusters(
                num_clusters=optimal_num_clusters, linkage=linkage, correlation=corr
            )
    ...
    def _get_optimal_number_of_clusters(
        self, correlation, asset_returns, linkage, num_reference_datasets=5
    ):
        """
        ...
        while True:
            ...
            original_cluster_assignments = fcluster(
                original_clusters, num_clusters, criterion="maxclust"
            )
            if (
                max(original_cluster_assignments) == max_number_of_clusters
                or max(original_cluster_assignments) > 10
            ):
                break
    ...
    def _calculate_expected_inertia(
        self, num_reference_datasets, asset_returns, num_clusters, linkage
    ):
        """
        ...
        for _ in range(num_reference_datasets):
            ...
            reference_cluster_assignments = fcluster(
                reference_clusters, num_clusters, criterion="maxclust"
            )
            inertia = self._compute_cluster_inertia(
    ...
    def _tree_clustering(self, correlation, linkage):
        ...
        distance_matrix = np.sqrt(2 * (1 - correlation).round(5))
        clusters = scipy_linkage(squareform(distance_matrix.values), method=linkage)
        clustering_inds = fcluster(
            clusters, self.optimal_num_clusters, criterion="maxclust"
        )
        cluster_children = {
            index - 1: []
            for index in range(min(clustering_inds), max(clustering_inds) + 1)
        }
    ...
    def _quasi_diagnalization(self, num_assets, curr_index):
        ...
        return self._quasi_diagnalization(
            num_assets, left
        ) + self._quasi_diagnalization(num_assets, right)

    def _recursive_bisection(
        self, asset_returns, covariance_matrix, assets, risk_measure
    ):
        """
        ...
        self._calculate_risk_contribution_of_clusters(
            clusters_contribution, risk_measure, covariance_matrix, asset_returns
        )
        ...
        self._calculate_final_portfolio_weights(
            risk_measure, clusters_weights, covariance_matrix, asset_returns
        )
    ...
    def _calculate_final_portfolio_weights(
        self, risk_measure, clusters_weights, covariance_matrix, asset_returns
    ):
        """
        ...
        for cluster_index in range(self.optimal_num_clusters):
            ...
            cluster_covariance = covariance_matrix.iloc[
                cluster_asset_indices, cluster_asset_indices
            ]
            ...
            self.weights[cluster_asset_indices] = (
                parity_weights * clusters_weights[cluster_index]
            )

    def _calculate_naive_risk_parity(
        self, cluster_index, risk_measure, covariance, asset_returns
    ):
    ...
    def _calculate_risk_contribution_of_clusters(
        self, clusters_contribution, risk_measure, covariance_matrix, asset_returns
    ):
        """
        ...
        for cluster_index in range(self.optimal_num_clusters):
            ...
            if risk_measure == "variance":
                clusters_contribution[cluster_index] = self._get_cluster_variance(
                ...
                clusters_contribution[
                    cluster_index
                ] = self._get_cluster_expected_shortfall(
                    asset_returns, cluster_asset_indices
                )
            elif risk_measure == "conditional_drawdown_risk":
                clusters_contribution[
                    cluster_index
                ] = self._get_cluster_conditional_drawdown_at_risk(
                    asset_returns=asset_returns, cluster_indices=cluster_asset_indices
                )
    ...
    def _get_cluster_conditional_drawdown_at_risk(self, asset_returns, cluster_indices):
        ...
        portfolio_returns = cluster_asset_returns @ parity_weights
        cluster_conditional_drawdown = (
            self.risk_metrics.calculate_conditional_drawdown_risk(
                returns=portfolio_returns, confidence_level=self.confidence_level
            )
        )
