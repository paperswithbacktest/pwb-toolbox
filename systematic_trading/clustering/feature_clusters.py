...
from systematic_trading.clustering.onc import get_onc_clusters
...
def get_feature_clusters(
    ...
    if n_clusters is None and (distance_metric is None or linkage_method is None):
        return list(
            get_onc_clusters(dep_matrix.fillna(0))[1].values()
        )  # Get optimal number of clusters
    if distance_metric is not None and (
        linkage_method is not None and n_clusters is None
    ):
    ...
    clustered_subsets = [
        [f for c, f in zip(clusters, X.columns) if c == ci]
...
def _cluster_transformation(
    X: pd.DataFrame, clusters: dict, feats_to_transform: list
) -> pd.DataFrame:
    """
    Machine Learning for Asset Managers
...
def _check_for_low_silhouette_scores(
    X: pd.DataFrame, critical_threshold: float = 0.0
) -> pd.DataFrame:
    """
    Machine Learning for Asset Managers
    Snippet 6.5.2.1 , page 85. Step 1: Features Clustering (last paragraph)
    ...
    if len(low_silh_feat) > 0:
        print(
            f"{len(low_silh_feat)} feature/s found with low silhouette score {low_silh_feat}. Returning the transformed dataset"
        )
