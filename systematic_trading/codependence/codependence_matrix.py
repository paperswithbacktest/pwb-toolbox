...
from systematic_trading.codependence.correlation import distance_correlation
...
def get_dependence_matrix(
    ...
    if dependence_method == "information_variation":
        ...
        dep_function = lambda x, y: gnpr_distance(
            x, y, theta=theta, bandwidth=bandwidth
        )
    else:
        raise ValueError(
            f"{dependence_method} is not a valid method. Please use one of the supported methods \
    ...
    dependence_matrix = np.array(
        [
            [
                ...
                0.5 * dep_function(np_df[i], np_df[j])
                if j == i
                else 0  # Make upper triangle 0 to fill it later on
                for j in range(n)
            ]
            for i in range(n)
        ]
    ...
    dependence_df = pd.DataFrame(
        data=dependence_matrix, index=features_cols, columns=features_cols
    )
...
def get_distance_matrix(
    X: pd.DataFrame, distance_metric: str = "angular"
) -> pd.DataFrame:
    """
    ...
    if distance_metric == "angular":
        distfun = lambda x: ((1 - x).round(5) / 2.0) ** 0.5
    elif distance_metric == "abs_angular":
        distfun = lambda x: ((1 - abs(x)).round(5) / 2.0) ** 0.5
    elif distance_metric == "squared_angular":
        distfun = lambda x: ((1 - x**2).round(5) / 2.0) ** 0.5
    else:
        raise ValueError(
            f"{distance_metric} is a unknown distance metric. Please use one of the supported methods \
                            listed in the docsting."
        )
