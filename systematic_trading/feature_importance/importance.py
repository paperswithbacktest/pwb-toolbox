...
from systematic_trading.cross_validation.cross_validation import ml_cross_val_score
    ...
    * Sklearn’s RandomForest class implements MDI as the default feature importance score. This choice is likely
    ...
    if clustered_subsets is not None:
        ...
        for subset in clustered_subsets:  # Iterating over each cluster
            ...
            importance.loc[subset, "std"] = (
                subset_feat_imp.std() * subset_feat_imp.shape[0] ** -0.5
            )
    else:
...
def mean_decrease_accuracy(
    ...
    * The CV must be purged and embargoed.
    ...
    fold_metrics_values, features_metrics_values = pd.Series(
        dtype="float64"
    ...
    for i, (train, test) in enumerate(cv_gen.split(X=X)):
        ...
        if scoring == log_loss:
            ...
            fold_metrics_values.loc[i] = -scoring(
                y.iloc[test],
                prob,
            ...
            fold_metrics_values.loc[i] = scoring(
                y.iloc[test], pred, sample_weight=sample_weight_score[test]
            )
        ...
        for j in feature_sets:
            ...
            for j_i in j:
                rs_obj.shuffle(
                    X1_[j_i].values
                )  # Permutation of a single column for MDA or through the whole subset for CFI
            if scoring == log_loss:
                prob = fit.predict_proba(X1_)
                features_metrics_values.loc[i, j] = -scoring(
                    y.iloc[test],
                    prob,
                    sample_weight=sample_weight_score[test],
                    labels=model.classes_,
...
def single_feature_importance(
    ...
    * Like MDA, it can conclude that all features are unimportant, because performance is evaluated via OOS CV.
    ...
    for feat in feature_names:
        feat_cross_val_scores = ml_cross_val_score(
            clf,
            X=X[[feat]],
            y=y,
        ...
        imp.loc[feat, "std"] = (
            feat_cross_val_scores.std() * feat_cross_val_scores.shape[0] ** -0.5
        )
    return imp


def plot_feature_importance(
    importance_df, oob_score, oos_score, save_fig=False, output_path=None
):
    """
    ...
    plt.title(
        "Feature importance. OOB Score:{}; OOS score:{}".format(
            round(oob_score, 4), round(oos_score, 4)
        )
    )
