...
from systematic_trading.microstructural_features.encoding import encode_array
...
from systematic_trading.util.misc import crop_data_frame_in_batches
...
class MicrostructuralFeaturesGenerator:
    ...
    def __init__(
        ...
        if isinstance(trades_input, str):
            self.generator_object = pd.read_csv(
                trades_input, chunksize=batch_size, parse_dates=[0]
            )
            ...
            raise ValueError(
                "trades_input is neither string(path to a csv file) nor pd.DataFrame"
            )
    ...
    def _get_bar_features(self, date_time: pd.Timestamp, list_bars: list) -> list:
        ...
        features.extend(
            get_trades_based_kyle_lambda(
                self.price_diff, self.trade_size, self.tick_rule
            )
        )  # Kyle lambda
        features.extend(
        ...
        features.extend(
            get_trades_based_hasbrouck_lambda(
                self.log_ret, self.dollar_size, self.tick_rule
            )
        )  # Hasbrouck lambda
