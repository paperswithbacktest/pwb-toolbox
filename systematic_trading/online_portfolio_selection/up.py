...
class UP(OLPS):
    ...
    def __init__(self, number_of_experts, weighted="hist_performance", k=1):
        ...
        self.expert_portfolio_returns = (
            None  # (np.array) All experts' portfolio returns over time.
        )
    ...
    def _initialize(self, asset_prices, weights, resample_by):
        ...
        self.expert_portfolio_returns = np.zeros(
            (self.length_of_time, self.number_of_experts)
        )
        # Set all experts' weights
        self.expert_all_weights = np.zeros(
            (self.number_of_experts, self.length_of_time, self.number_of_assets)
        )
    ...
    def _calculate_weights_on_experts(self):
        ...
        if self.weighted == "hist_performance":
            ...
            expert_returns_ratio = np.apply_along_axis(
                lambda x: x / np.sum(x), 1, self.expert_portfolio_returns
            )
            # Initial weights are evenly distributed among all experts.
            expert_returns_ratio = np.vstack(
                (self._uniform_experts(), expert_returns_ratio)
            )
        ...
        # If capital allocation is based on uniform weights.
            ...
            # Equal allocation.
            uniform_ratio = (
                np.ones(self.expert_portfolio_returns.shape) / self.number_of_experts
            )
            # Initial weights are evenly distributed among all experts.
        ...
        # If capital allocation is based on top-K experts.
            ...
            top_k = np.apply_along_axis(
                lambda x: np.argpartition(x, -self.k)[-self.k :],
                1,
                self.expert_portfolio_returns,
            )
            ...
            # Initial weights are evenly distributed among all experts.
            top_k_distribution = np.vstack(
                (self._uniform_experts(), top_k_distribution)
            )
            self.weights_on_experts = top_k_distribution
        else:
            raise ValueError(
                "Please put in 'hist_performance' for Historical Performance, "
    ...
    def _calculate_all_weights(self):
        ...
        d_shape = (
            self.weights_on_experts[:-1].shape[:1] + self.expert_all_weights.shape[1:]
        )
        reshaped_all_weights = self.expert_all_weights.reshape(
            self.expert_all_weights.shape[0], -1
        )
        weight_change = np.dot(
            self.weights_on_experts[:-1], reshaped_all_weights
        ).reshape(d_shape)
    ...
    def _print_progress(
        self, iteration, prefix="", suffix="", decimals=1, bar_length=50
    ):
        ...
        filled_length = int(
            round(bar_length * iteration / float(self.number_of_experts))
        )
        # Fill the bar.
