...
class RMR(OLPS):
    ...
    def _update_weight(self, time):
        ...
        predicted_deviation = current_prediction - np.ones(
            self.number_of_assets
        ) * np.mean(current_prediction)
        ...
        if norm2 == 0:
            return self.weights
        alpha = np.minimum(
            0, (current_prediction * self.weights - self.epsilon) / norm2
        )
    ...
    def _calculate_predicted_relatives(self, time):
        ...
        for _ in range(self.n_iteration - 1):
            ...
            if curr_prediction.size == 0 or np.linalg.norm(
                prev_prediction - curr_prediction, ord=1
    ...
    def _transform(old_mu, price_window):
        ...
        gamma = np.linalg.norm(
            np.sum(
                np.apply_along_axis(lambda x: x / np.linalg.norm(x, ord=1), 1, non_mu),
                axis=0,
            ),
            ord=1,
        )
        ...
        with np.errstate(invalid="ignore"):
            next_mu = (
                np.maximum(0, 1 - eta / gamma) * tilde
                + np.minimum(1, eta / gamma) * old_mu
            )
