...
class SCORN(CORN):
    ...
    def _scorn_optimize(self, similar, opposite):
        ...
        def _objective(weight):
            return -np.sum(np.log(np.dot(similar, weight))) + np.sum(
                np.log(np.dot(opposite, weight))
            )
        ...
        def _derivative(weight):
            ...
            return -np.dot(1 / similar_returns, similar) + np.dot(
                1 / opposite_returns, opposite
            )
