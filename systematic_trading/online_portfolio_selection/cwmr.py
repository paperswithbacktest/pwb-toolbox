...
class CWMR(OLPS):
    ...
    def _update_weight(self, time):
        ...
        new_v = np.dot(
            np.dot(curr_relative_return, self.sigma), curr_relative_return.T
        )[0][0]

        # Calculate weighted variance.
        new_w = np.dot(
            np.dot(curr_relative_return, self.sigma), np.identity(self.number_of_assets)
        )[0]
        ...
        # Update mu.
        self.mu_dist -= lambd * np.dot(
            curr_relative_return - mean_x, self.sigma
        ).reshape((self.number_of_assets,))

        # Calculate new sigma.
        if self.method == "sd":
            # Component for new variance calculation.
            sqrt_u = (
                -lambd * self.theta * new_v
                + np.sqrt(lambd**2 * self.theta**2 * new_v**2 + 4 * new_v)
            ) / 2
            # Update variance.
            self.sigma = np.linalg.pinv(
                np.linalg.pinv(self.sigma)
                + lambd * self.theta / sqrt_u * np.diag(curr_relative_return) ** 2
            )
        ...
        if self.method == "var":
            ...
            self.sigma = np.linalg.pinv(
                np.linalg.pinv(self.sigma)
                + 2 * lambd * self.theta * np.diag(curr_relative_return) ** 2
            )
