...
def trend_scanning_labels(
    ...
    for index in t_events:
        subset = price_series.loc[index:].iloc[
            :look_forward_window
        ]  # Take t:t+L window
        if subset.shape[0] >= look_forward_window:
            ...
            max_abs_t_value = (
                -np.inf
            )  # Maximum abs t-value of b_1 coefficient among l values
    ...
    labels.loc[:, "ret"] = (
        price_series.reindex(labels.t1).values
        / price_series.reindex(labels.index).values
        - 1
    )
